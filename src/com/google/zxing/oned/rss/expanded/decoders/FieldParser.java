...
_L2:
        ...
        do
        {
            ...
            if(TWO_DIGIT_DATA_LENGTH[i][0].equals(s1))
            {
                if(TWO_DIGIT_DATA_LENGTH[i][1] == VARIABLE_LENGTH)
                    s2 = processVariableAI(2, ((Integer)TWO_DIGIT_DATA_LENGTH[i][2]).intValue(), s);
                else
                    s2 = processFixedAI(2, ((Integer)TWO_DIGIT_DATA_LENGTH[i][1]).intValue(), s);
        ...
        j = 0;
_L5:
        if(j < THREE_DIGIT_DATA_LENGTH.length) goto _L4; else goto _L3
_L3:
        int k = 0;
_L6:
...
_L4:
        if(THREE_DIGIT_DATA_LENGTH[j][0].equals(s3))
        {
            if(THREE_DIGIT_DATA_LENGTH[j][1] == VARIABLE_LENGTH)
                s2 = processVariableAI(3, ((Integer)THREE_DIGIT_DATA_LENGTH[j][2]).intValue(), s);
            else
                s2 = processFixedAI(3, ((Integer)THREE_DIGIT_DATA_LENGTH[j][1]).intValue(), s);
            continue; /* Loop/switch isn't completed */
        }
        j++;
          goto _L5
        if(THREE_DIGIT_PLUS_DIGIT_DATA_LENGTH[k][0].equals(s3))
        {
            if(THREE_DIGIT_PLUS_DIGIT_DATA_LENGTH[k][1] == VARIABLE_LENGTH)
                s2 = processVariableAI(4, ((Integer)THREE_DIGIT_PLUS_DIGIT_DATA_LENGTH[k][2]).intValue(), s);
            else
                s2 = processFixedAI(4, ((Integer)THREE_DIGIT_PLUS_DIGIT_DATA_LENGTH[k][1]).intValue(), s);
            continue; /* Loop/switch isn't completed */
        }
        k++;
          goto _L6
...
label0:
        {
            if(l >= FOUR_DIGIT_DATA_LENGTH.length)
                throw NotFoundException.getNotFoundInstance();
            if(!FOUR_DIGIT_DATA_LENGTH[l][0].equals(s4))
                break label0;
            if(FOUR_DIGIT_DATA_LENGTH[l][1] == VARIABLE_LENGTH)
                s2 = processVariableAI(4, ((Integer)FOUR_DIGIT_DATA_LENGTH[l][2]).intValue(), s);
            else
                s2 = processFixedAI(4, ((Integer)FOUR_DIGIT_DATA_LENGTH[l][1]).intValue(), s);
        }
...
_L10:
        ...
        throws NotFoundException
    {
        ...
        } else
        {
            ...
            return (new StringBuilder(String.valueOf('('))).append(s1).append(')').append(s2).append(parseFieldsInGeneralPurpose(s3)).toString();
        }
    }
