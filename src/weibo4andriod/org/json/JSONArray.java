...
public class JSONArray
{
        ...
        throws JSONException
    {
        ...
        if(obj.getClass().isArray())
        {
            int i = Array.getLength(obj);
            for(int j = 0; j < i; j++)
                put(new JSONObject(Array.get(obj, j), flag));
    ...
    public JSONArray(Collection collection, boolean flag)
    {
        ...
        if(collection != null)
        {
            for(Iterator iterator = collection.iterator(); iterator.hasNext(); myArrayList.add(new JSONObject(iterator.next(), flag)));
        }
    }
...
_L8:
        throw jsontokener.syntaxError((new StringBuilder()).append("Expected a '").append(new Character(c1)).append("'").toString());
    }
...
_L1:
    ...
    {
        String s1 = (new StringBuilder()).append('[').append(join(",")).append(']').toString();
        String s = s1;
_L2:
        return s;
        Exception exception;
